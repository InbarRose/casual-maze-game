 ...
 *
 * Enforces:
 * - 100% non-bypassability: gates, levers, teleporters, and puzzle gates are mandatory chokepoints.
 * - Active obstacle interactions: locked blocking, key collection & consumption, minigame validation,
 *   tile mutations, and 100% obstacle clearance audits.
 */
    ...
    it(`plays through Level ${level.id} (${level.title}) to victory via game loop simulation`, () => {
      ...
      for (let i = 1; i < path.length; i++) {
        ...
        if (puzzleGate && !puzzleGate.isUnlocked) {
          // Verify locked puzzle gate strictly blocks movement
        ...
        if (targetDoor && !targetDoor.isOpen) {
          // Assert that player has matching key before moving through
          assert(
          ...
          // Verify that without the key, movement would be blocked
          const blockedCheck = CollisionEngine.checkMove(
            gameLoop.player.gridX,
            gameLoop.player.gridY,
            step.x,
        ...
        // If door was passed, verify it is open and key was consumed
        if (targetDoor) {
